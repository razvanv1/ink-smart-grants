/**
 * Agent Task API service — mock implementation.
 * Replace with real API calls when OpenClaw backend is ready.
 * All functions follow the documented API contract.
 */
...
const mockTasks: AgentTask[] = [
  {
    ...
    updatedAt: '2026-03-20T09:18:42Z',
    progress: 100,
  ...
  },
  {
    ...
    status: 'running',
    createdAt: '2026-03-21T14:22:00Z',
    updatedAt: '2026-03-22T08:05:12Z',
    progress: 63,
    currentStep: 'Drafting impact assessment section',
  ...
  },
  {
    ...
    createdAt: '2026-03-21T16:44:00Z',
    updatedAt: '2026-03-22T07:30:00Z',
    progress: 78,
    currentStep: 'Awaiting budget breakdown approval',
  ...
  },
  {
    ...
    updatedAt: '2026-03-19T11:14:55Z',
    progress: 22,
    currentStep: 'Failed at partner database lookup',
  ...
  },
  {
    ...
    updatedAt: '2026-03-22T06:03:18Z',
    progress: 100,
    currentStep: 'Completed',
...
export async function submitReview(taskId: string, review: ReviewDecision): Promise<AgentTask> {
  await delay(500);
  const task = mockTasks.find(t => t.taskId === taskId);
  if (!task) throw new Error(`Task ${taskId} not found`);
  if (review.decision === 'approved') {
    task.status = 'running';
    task.currentStep = 'Resuming after approval';
  } else {
    task.status = 'running';
    task.currentStep = `Revising: ${review.notes || 'changes requested'}`;
  }
